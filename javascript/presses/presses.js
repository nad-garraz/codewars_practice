...
Likewise, the phone doesn't distinguish betweeen upper and lowercase characters (but you should allow your module to accept input in either form, for conveniience).
Tested phrases contain letters (A-Z and a-z), di0its (0-9), and special characters # and *.
*/

const arr = ['1ADGJMPTW*# ', 'BEHKNQUX0', 'CFILORVY', 'ZS234568', '79'];
function presses(phrase) {
  return phrase.toUpperCase().split('').reduce((ans, c) => {
      for (let i = 0; i < arr.length; i++) if (arr[i].includes(c)) return ans + i + 1;
    }, 0);
}

console.log(presses('lol'));
console.log(presses('abcdefghijklmnopqrstuvwxyz'));
