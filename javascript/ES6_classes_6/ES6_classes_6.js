...
class File {
  constructor(fullName, content) {
    ...
    this._content = content.replace(/^\s*\n|\s*\n$/,'').split("\n");
  ...
  write(newLine) {
    return this._content.push(newLine.replace(/^\s*\n|\s*\n$/g,''));
  }

  gets() {
    this._sIndex++;
    if (this._content[this._sIndex] === '' ){this._sIndex++;}
    return this._content[this._sIndex];
  }

  getc() {
    this._cIndex++;
    // return this.getContents().replace(/^\s*\n|\s*\n$/g,'')[this._cIndex];
    return this.getContents()[this._cIndex];
  }
}
